...
//{
//   const vec3 x_i = vec3(particle[index].position.x,
...
//{
//   return mat3(cos(alpha_z), -sin(alpha_z), 0, sin(alpha_z), cos(alpha_z), 0,
//   0,
//               0, 1);
// }
...
//   return pow(E, -pow(abs_x / Essentials::KernelRadius, 2. / 3.) / (2 * b *
//   b)) /
//          pow(2 * PI * b * b, 1. / 2.);
// }
//
...
//   return v_i +
//          (v_j - v_i) *
//              vec3(((x_ij - x_i) * (x_j - x_i)).x / pow(distance(x_j, x_i),
//              2),
//                   ((x_ij - x_i) * (x_j - x_i)).y / pow(distance(x_j, x_i),
//                   2),
//                   ((x_ij - x_i) * (x_j - x_i)).z / pow(distance(x_j, x_i),
//                   2));
// }
//
...
// }
//
// vec3 HydroTest::CalculateNearPsi(const uint32_t index_i, const uint32_t
// index_x)
//{
//   mat3 E_x = mat3(0);
//   for (uint32_t j = 0; j < Essentials::MaxNeighbours &&
...
//
//   Vector gradCrossW_x = Vector(dot(gradW_x[0], x_mean_x), gradW_x[1] *
//   x_mean_x,
...
//                   particle[index_j].position.z);
//   vec3 x_ij = vec3((x_i + x_j).x / 2., (x_i + x_j).y / 2., (x_i + x_j).z
//   / 2.); vec3 v_frameij = CalculateFrameVelocity(index_i, index_j);
//
...
//                   ? 1
//                   : sqrt(1 + (gamma + 1) / (2. * gamma) * (p_hash / p_l -
//                   1));
//   float q_r = p_hash / p_r <= 1
//                   ? 1
//                   : sqrt(1 + (gamma + 1) / (2. * gamma) * (p_hash / p_r -
//                   1));
//
...
//
//   vec3 U_l_hash =
//       vec3(ro_l_hash, ro_l_hash * u_hash, (Q.z + S_m * p_l_hash) / (S_l -
//       S_m));
//   vec3 U_r_hash =
//       vec3(ro_r_hash, ro_r_hash * u_hash, (R.z + S_m * p_r_hash) / (S_r -
//       S_m));
//
...
//   {
//     F_i2 += vec3((CourantNumbers[k] - CourantNumbers[k - 1]) / 2.) *
//     F_i2_k[k];
//   }
//   /////////////////////////////////////FLUX IJ
//   ///DEBOOST//////////////////////////////////////////
...
//         mat3(0, -rot_m.z, rot_m.y, rot_m.z, 0, -rot_m.x, -rot_m.y, rot_m.x,
//         0);
...
//{
//   vec3 vel_i = vec3(particle[index_i].velocity.x,
//   particle[index_i].velocity.y,
//                     particle[index_i].velocity.z);
//   Vector U_n =
