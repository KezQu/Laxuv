...
void HydroTest::FindNeighbours(uint32_t index, uint32_t MaxParticles) {
    ...
    for (int j = 0; j < Essentials::MaxNeighbours; j++) {
        particle[index].neighbours[j] = 0xffffffff;
    }
    uint32_t neighbourCount = 0;
    for (uint32_t j = 0; j < MaxParticles && neighbourCount < Essentials::MaxNeighbours; j++) {
...
float HydroTest::CalculateKernelWeight(vec3 x) {
    float b = 0.4f;
    float abs_x = length(x);
    return (pow(E, -(abs_x * abs_x) / (2 * b * b * Essentials::KernelRadius * Essentials::KernelRadius)) * Essentials::KernelRadius) / pow(2 * PI * b * b, 3 / 2);
}
...
float HydroTest::CalculateOmega(uint32_t index_x) {
    ...
    for (uint32_t j = 0; j < Essentials::MaxNeighbours && particle[index_x].neighbours[j] != 0xffffffff; j++) {
...
vec3 HydroTest::CalculateNearPsi(uint32_t index_i, uint32_t index_x) {
...
GradVector HydroTest::CalculateGradW(uint32_t index_x, Vector Wp) {
...
mat3 HydroTest::ConvertToUpper(mat3 mtx) {
    mat3 upper_mtx = mat3(0);
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
            upper_mtx[i][j] = mtx[i][j];
        }
    }
    if (upper_mtx[0][0] == 0) {
        vec3 swap = upper_mtx[0];
        upper_mtx[0] = upper_mtx[2];
        upper_mtx[2] = swap;
    }
    if (upper_mtx[1][1] == 0) {
        vec3 swap = upper_mtx[1];
        upper_mtx[1] = upper_mtx[2];
        upper_mtx[2] = swap;
    }
    if (upper_mtx[0][0] != 0) {
        auto test1 = upper_mtx[0][0] * upper_mtx[1][0] / upper_mtx[0][0];
        auto test2 = upper_mtx[0][1] * upper_mtx[1][0] / upper_mtx[0][0];
        auto test3 = upper_mtx[0][2] * upper_mtx[1][0] / upper_mtx[0][0];

        upper_mtx[1][0] -= test1;
        upper_mtx[1][1] -= test2;
        upper_mtx[1][2] -= test3;

        test1= upper_mtx[0][0] * upper_mtx[2][0] / upper_mtx[0][0];
        test2= upper_mtx[0][1] * upper_mtx[2][0] / upper_mtx[0][0];
        test3= upper_mtx[0][2] * upper_mtx[2][0] / upper_mtx[0][0];
    ...
    if (upper_mtx[1][1] != 0) {
        auto test1 = upper_mtx[1][1] * upper_mtx[2][1] / upper_mtx[1][1];
        auto test2 = upper_mtx[1][2] * upper_mtx[2][1] / upper_mtx[1][1];
        upper_mtx[2][1] -= test1;
        upper_mtx[2][2] -= test2;
    }
    return upper_mtx;
...
Vector HydroTest::CalculateTimeDerivativeOfW(uint32_t index_x, Vector Wp) {
	...
	mat3 upper_Wp_y = ConvertToUpper(gradWp.y);
    const vec3 Wp_y_diagonal = vec3(upper_Wp_y[0][0], upper_Wp_y[1][1], upper_Wp_y[2][2]);
    dW_dt.x = -(dot(v, gradWp.x) + dot(vec3(density), Wp_y_diagonal));
    dW_dt.y = -(v * gradWp.y + vec3(1. / density) * gradWp.z);
    dW_dt.z = -(dot(vec3(gamma * pressure), Wp_y_diagonal) + dot(v, gradWp.z));

    return dW_dt;
}

Vector HydroTest::PrepareRiemmanProblemSide(uint32_t index_x, vec3 v_frame, vec3 x_mean_x, vec3* dir_vector) {
...
Vector HydroTest::CalculateReimannProblem(uint32_t index_i, uint32_t index_j) {
    const float dt = 0.01667f;
    const mat3 I = mat3(1, 0, 0,
        0, 1, 0,
        0, 0, 1);
    ...
    /////////////////////////////////////FLUX IJ DEBOOST//////////////////////////////////////////
    const dvec3 x_versor = vec3(1, 0, 0);
    const dvec3 dir_vector_m = dir_vector_l + dir_vector_l;
    dvec3 rot_m = cross(dir_vector_m, x_versor);
    rot_m = length(rot_m) > 0 ? normalize(rot_m) : rot_m;
    const double cos_angle_m = dot(dir_vector_m, x_versor) / length(dir_vector_m);
    const dmat3 rot_helper_m = dmat3(0, -rot_m.z, rot_m.y,
                                   rot_m.z, 0, -rot_m.x,
                                   -rot_m.y, rot_m.x, 0);
    const dmat3 rotMtx_m = dmat3(I) + sqrt(1 - cos_angle_m * cos_angle_m) * rot_helper_m + (1- cos_angle_m) * rot_helper_m * rot_helper_m;
	dvec3 test_m = dir_vector_m * rotMtx_m;
    
    const dvec3 dir_versor_m = length(dir_vector_m) > 0 ? normalize(dir_vector_m) : dir_vector_m;
    Vector Fp_ij = Vector(
        F_i2.x, 
        dir_versor_m * (vec3(F_i2.y, 0, 0) * inverse(rotMtx_m)),
        F_i2.z);

    Vector F_ij = Vector(
        Fp_ij.x, 
        Fp_ij.y + v_frameij * Fp_ij.x, 
        Fp_ij.z + pow(length(v_frameij), 2) * Fp_ij.x / 2. + dot(v_frameij, Fp_ij.y));
...
Vector HydroTest::GenerateHydrodynamics(uint32_t index_i) {
    ...
    Vector U_n = Vector(particle[index_i].VolumeDensityPressureMass.y,
        ...
        particle[index_i].VolumeDensityPressureMass.y * pow(length(vel_i), 2) / 2);
    ...
    for (int j = 0; j < Essentials::MaxNeighbours && particle[index_i].neighbours[j] != 0xffffffff; j++) {
