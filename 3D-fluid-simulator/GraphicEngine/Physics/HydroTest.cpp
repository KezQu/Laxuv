...
PhysicsProperties particle[1000];
...
PhysicsProperties* Init() {
	...
	{
		particle[i].position = Qube(i, 1000);
...
float CalculateOmega(uint32_t index_x) {
	...
	for (uint32_t j = 0; j < MaxNeighbours && particle[index_x].neighbours[j] != 0xffffffff; j++) {
		omega += CalculateKernelWeight(glm::vec3(particle[index_x].position.x, particle[index_x].position.y, particle[index_x].position.z) - glm::vec3(particle[j].position.x, particle[j].position.y, particle[j].position.z));
	}
	return omega;
}

float CalculatePsi(uint32_t index_i, uint32_t index_x) {
	return CalculateKernelWeight(glm::vec3(particle[index_x].position.x, particle[index_x].position.y, particle[index_x].position.z) - glm::vec3(particle[index_i].position.x, particle[index_i].position.y, particle[index_i].position.z)) / CalculateOmega(index_x);
}
...
glm::vec3 CalculateNearPsi(uint32_t index_i, uint32_t index_x) {
	...
	for (uint32_t j = 0; j < MaxNeighbours && particle[index_x].neighbours[j] != 0xffffffff; j++) {
		E += glm::outerProduct(glm::vec3(particle[j].position.x, particle[j].position.y, particle[j].position.z) - glm::vec3(particle[index_x].position.x, particle[index_x].position.y, particle[index_x].position.z),
			glm::vec3(particle[j].position.x, particle[j].position.y, particle[j].position.z) - glm::vec3(particle[index_x].position.x, particle[index_x].position.y, particle[index_x].position.z)) *
...
Vector CalculateReimannProblem(uint32_t index_i, uint32_t index_j) {
	...
	glm::vec3 direction_versor = normalize(Wpp_r.y);
...
Vector GenerateHydrodynamics(uint32_t index_i) {
	...
	for (int j = 0; j < MaxNeighbours && particle[index_i].neighbours[j] != 0xffffffff; j++) {
		...
		glm::vec3 nearPsi_ji = CalculateNearPsi(j, index_i);
		glm::vec3 nearPsi_ij = CalculateNearPsi(index_i, j);
