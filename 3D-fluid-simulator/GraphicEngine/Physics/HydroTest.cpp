...
void HydroTest::FindNeighbours(uint32_t index, uint32_t MaxParticles) {
    const vec3 x_i = vec3(particle[index].position.x, particle[index].position.y, particle[index].position.z);
    ...
    for (uint32_t j = 0; j < MaxParticles && neighbourCount < Essentials::MaxNeighbours; j++) {
        const vec3 x_j = vec3(particle[j].position.x, particle[j].position.y, particle[j].position.z);
...
float HydroTest::CalculateKernelWeight(vec3 x) {
    ...
    return pow(E, -pow(abs_x / Essentials::KernelRadius, 2. / 3.) / (2 * b * b)) / pow(2 * PI * b * b, 1. / 2.);
}

vec3 HydroTest::CalculateFrameVelocity(uint32_t index_i, uint32_t index_j) {
    const vec3 v_i = vec3(particle[index_i].velocity.x, particle[index_i].velocity.y, particle[index_i].velocity.z);
    const vec3 v_j = vec3(particle[index_j].velocity.x, particle[index_j].velocity.y, particle[index_j].velocity.z);
    const vec3 x_i = vec3(particle[index_i].position.x, particle[index_i].position.y, particle[index_i].position.z);
    const vec3 x_j = vec3(particle[index_j].position.x, particle[index_j].position.y, particle[index_j].position.z);
    const vec3 x_ij = vec3((x_i.x + x_j.x) / 2., (x_i.y + x_j.y) / 2., (x_i.z + x_j.z) / 2.);
...
float HydroTest::CalculateOmega(uint32_t index_x) {
    ...
    for (uint32_t j = 0; j < Essentials::MaxNeighbours && particle[index_x].neighbours[j] != MaxValueNeighbour; j++) {
...
vec3 HydroTest::CalculateNearPsi(const uint32_t index_i, const uint32_t index_x) {
    ...
    for (uint32_t j = 0; j < Essentials::MaxNeighbours && particle[index_x].neighbours[j] != MaxValueNeighbour; j++) {
        const  uint32_t neighbour_index = particle[index_x].neighbours[j];
        const vec3 neighbour_x_diff = vec3(particle[neighbour_index].position.x, particle[neighbour_index].position.y, particle[neighbour_index].position.z) -
            vec3(particle[index_x].position.x, particle[index_x].position.y, particle[index_x].position.z);
        E_x += outerProduct(neighbour_x_diff, neighbour_x_diff) * CalculatePsi(neighbour_index, index_x);
    }
    const  mat3 B_x = inverse(E_x);
    const vec3 nearPsi_j = B_x * (vec3(particle[index_i].position.x, particle[index_i].position.y, particle[index_i].position.z) - vec3(particle[index_x].position.x, particle[index_x].position.y, particle[index_x].position.z)) * CalculatePsi(index_i, index_x);
    return nearPsi_j;
}
...
mat3 HydroTest::CalculateGradW(const uint32_t index_x, const Vector Wp) {
    ...
    for (uint32_t j = 0; j < Essentials::MaxNeighbours && particle[index_x].neighbours[j] != MaxValueNeighbour; j++) {
        ...
        const Vector Wp_temp_j = Vector(particle[neighbour_index].VolumeDensityPressureMass.y,
...
Vector HydroTest::CalculateTimeDerivativeOfW(const uint32_t index_x, const Vector Wp) {
...
Vector HydroTest::GenerateHydrodynamics(uint32_t index_i) {
    ...
    for (int j = 0; j < Essentials::MaxNeighbours && particle[index_i].neighbours[j] != MaxValueNeighbour; j++) {
